 ...
 */
export const getLyricsMeaning = async ({ lyrics, songId, title, artist, customInstructions }) => {
  try {
    ...
    // Log source for debugging/observability
    const isCached = data.cached === true;
    if (isCached) {
      // eslint-disable-next-line no-console
      console.log('[Meaning] Retrieved from cache (Supabase).');
    } else {
      // eslint-disable-next-line no-console
      console.log('[Meaning] Generated by Gemini and stored to cache.');
    }

    return { data: data.data, cached: isCached };
  } catch (error) {
